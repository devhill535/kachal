...
bot.login("ODI4MjcwNTU2NzU4NTQwMzQ4.YGnJEQ.rldIiIDwPWwHr3hLt2DE6N8r8HQ")
global.mongoose = require('mongoose')
mongoose.connect("mongodb+srv://sako:sakran12345+@cluster0.7nthm.mongodb.net/myFirstDatabase?retryWrites=true&w=majority", { useNewUrlParser: true, useUnifiedTopology: true }).then(() => {
...
bot.on("ready", async () => {
 let channel = bot.channels.cache.get("888817730478829738");
     ...
     if (usersMap.has(message.author.id)) {
       ...
       if (difference > DIFF) {
         ...
         if (parseInt(msgCount) >= LIMIT) {
          ...
          .setColor(Color)
          .setThumbnail(k.guild.iconURL())
          .setTitle(`<:punishment:837867514947174431> Actions in the server **${k.guild.name}**`)
          .setDescription(`${k.author.username} kicked because spaming in channel!`));
               });
             message.channel.bulkDelete(msgCount, true);
           }
         } else {
